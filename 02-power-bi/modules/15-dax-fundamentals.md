# Module 15: DAX Fundamentals

## DAX ဆိုတာဘာလဲ?

DAX (Data Analysis Expressions) သည် Power BI, Power Pivot, Analysis Services တို့တွင် အသုံးပြုသော formula/expression language ဖြစ်ပါသည်။ DAX ကို Excel formulas များနှင့်ဆင်တူသော်လည်း Power BI ၏ in-memory columnar engine အတွက်အထူးဒီဇိုင်းထုတ်ထားပါသည်။

### Measures vs Calculated Columns

| Feature | Measure | Calculated Column |
|---------|---------|-------------------|
| When Calculated | Query time (dynamic) | Data refresh time |
| Storage | Not stored (in-memory) | Stored in model (uses RAM) |
| Filter Context | Responds to filters | Fixed value |
| Performance | Better (compressed) | Higher memory usage |
| Use Case | Aggregations, KPIs | Row-level categorization |

```dax
// Measure — filter context ကိုတုံ့ပြန်သည်
Total Sales = SUM(Sales[Amount])

// Calculated Column — row context တွင်တွက်သည်
Full Name = Customers[FirstName] & " " & Customers[LastName]
```

### Evaluation Context

1. **Row Context** — Row တစ်ခုချင်းစီအတွက်တွက်ချက်ခြင်း (Calculated Columns, Iterators)
2. **Filter Context** — ယခု filter သက်ရောက်နေသော rows များပေါ်တွင်တွက်ချက်ခြင်း (Measures)

### DAX Functions Categories

| Category | Functions |
|----------|-----------|
| Aggregate | SUM, AVERAGE, COUNT, MIN, MAX, DISTINCTCOUNT |
...
| Date | YEAR, MONTH, DAY, WEEKDAY, TODAY |
...
1. **Too many visuals on one page** — 5-7 visuals max per page
2. **Wrong chart type** — Pie chart for > 5 categories, line chart f