# Module 16: DAX Aggregate Functions
...
Row တစ်ခုချင်းစီအတွက်တွက်ပြီးစုစည်းပါသည်။
...
DIVIDE(
    SUM(Sales[Profit]),
    SUM(Sales[Revenue]),
    0  // Alternative if division by zero
)
...
| Basic | Iterator | When to Use |
|-------|----------|-------------|
| SUM | SUMX | Row-level calculation လိုလျှင် |
| AVERAGE | AVERAGEX | Weighted average |
| COUNT | COUNTX | Conditional count |
| MIN | MINX | Row-by-row minimum |
| MAX | MAXX | Row-by-row maximum |
