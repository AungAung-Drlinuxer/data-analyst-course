...
Lakehouse = Data Lake + Data Warehouse ၏အားသာချက်များကိုပေါင်းစပ်ထားသော architecture ဖြစ်ပါသည်။

### Key Features

1. **Delta Tables** — ACID transactions, time travel (version history), schema evolution
2. **Shortcuts** — External data ကို copy မလုပ်ပဲ virtual reference ပြုလုပ်နိုင်
3. **SQL Endpoint** — T-SQL ဖြင့် query လုပ်နိုင် (auto-generated, read-only)
4. **File Support** — Parquet (best performance), CSV, JSON, Delta Lake

### OneLake Architecture

```
OneLake
    ├── Workspace A → Lakehouse 1
    │   ├── Tables/ (Delta Format)
    │   └── Files/ (Raw Files)
    └── Workspace B → Lakehouse 2
