...
Mass data transfer with schema mapping and data type conversion

### 2. Notebook Activity
Run Spark notebooks with parameter passing

### 3. Stored Procedure Activity
Execute SQL stored procedures with output parameter handling

### 4. If Condition Activity
```
If (condition) → True → Activity A
              → False → Activity B
```

### 5. ForEach Activity
Iterate through items with batch count and sequential/parallel execution
...
### 8. Fail Activity
Explicitly fail pipeline with custom error message

### Pipeline Control Flow Example

```
Start → ForEach (Customers) → If (Sales > 0)
    ├── Yes → Copy Data → Log Success
    └── No → Log Skip → Continue
→ End
```
