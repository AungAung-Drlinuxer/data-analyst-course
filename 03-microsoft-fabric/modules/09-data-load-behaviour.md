...
|------|-------------|-------------|
| Full Load | Data အားလုံးကိုပြန်ဖြည့်ခြင်း | First time, small tables |
| Incremental Load | အသစ်ထည့်ထားသော data များသာ | Large tables, daily jobs |
| Append | Row အသစ်များကိုထည့်ခြင်း | Logs, transactions |
...
WHERE ModifiedDate > (
    SELECT LastValue FROM Control.Watermark WHERE TableName = 'Sales'
);
```

### Data Consistency
- Delta Lake ACID guarantees (Atomic, Consistent, Isolated, Durable)
