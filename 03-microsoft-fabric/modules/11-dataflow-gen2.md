...
## DataFlow Gen 2 ဆိုတာဘာလဲ?

DataFlow Gen2 သည် Power Query Online ကိုအခြေခံထားသော low-code data transformation tool ဖြစ်ပါသည်။

### DataFlow vs Pipeline

| Feature | DataFlow Gen2 | Pipeline |
|---------|---------------|----------|
| Focus | Transformation | Orchestration |
| Interface | Power Query (Visual) | Drag & Drop |
| Language | M Language | JSON/Code |
| Complexity | Low-code | Pro-code |
| Use Case | Cleaning, Merging | Full ETL/ELT |

### Transformations

- Merge (Join) — Left, Right, Inner, Full, Fuzzy matching
- Append — Combine multiple tables vertically
- Pivot/Unpivot — Transform row ↔ column
- Replace values, Split columns, Remove duplicates

### Compute Settings

- Serverless: Auto-scale (pay per use)
- Provisioned: Dedicated compute resources
- Staging: Intermediate storage for optimization
