...
Apache Spark သည် big data processing engine ဖြစ်ပြီး Fabric တွင် managed Spark cluster အဖြစ်ပါဝင်ပါသည်။
...
df.write.format("delta").mode("overwrite").saveAsTable("agg_sales")

# Optimize (bin compaction)
spark.sql("OPTIMIZE agg_sales")

# Vacuum (clean old files)
spark.sql("VACUUM agg_sales RETAIN 168 HOURS")
```

### Notebooks
- Interactive development (Jupyter-style)
